...
void Pwnagotchi::detect() {
    // cool animation
    for (int i = 0; i < 5; ++i) {
        Serial.println("(0-o) scans for frnd...");
        Display::cleanDisplayFace("(0_o)");
        Display::attachSmallText("scans for frnd...");
        delay(1000);
        Serial.println("(o-0) Scanning for Pwnagotchi..");
        Display::cleanDisplayFace("(o_0)");
        Display::attachSmallText("where my frend...");
        delay(1000);
        Serial.println("(0-o) Scanning for Pwnagotchi...");
        Display::cleanDisplayFace("(0_o)");
        Display::attachSmallText("where art thou..");
        delay(1000);
        Serial.println(" ");
        delay(1000);
    }
    ...
    if (!pwnagotchiDetected) {
        ...
        Serial.println("(;-;) no friend found");
        Display::cleanDisplayFace("(w_w)");
        Display::attachSmallText("no frend found");
        Serial.println(" ");
    ...
    if (snifferPacket->payload[0] == 0x80) {
        ...
        if (src == "de:ad:be:ef:de:ad") {
            ...
            if (error) {
                ...
                Serial.print(" ");
                Display::cleanDisplayFace("(^-^)");
                Display::attachSmallText("friend name: " + (String) name);
                Display::attachSmallText("pwned nets: " + (String) pwndTot);
            }
        }
    }
}
